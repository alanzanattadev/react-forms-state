...
function dispatchOnProxy(rendered, state, onChange, statePath) {
  ...
  return React.cloneElement(
    ...
    enhance(rendered.props && rendered.props.children ? rendered.props.children : null, state, onChange, newPath)
  );
}

function enhance(rendered, state, onChange, statePath = '') {
  if (rendered == null || typeof rendered == "string" || typeof rendered == "number" || typeof rendered == "function")
    return rendered;
  ...
  else if (rendered.props && rendered.props._inputField && typeof rendered.props.name == 'string' && rendered.props.name.length > 0) {
    return dispatchOnProxy(rendered, state, onChange, statePath);
  } else {
    return React.cloneElement(
      rendered,
      rendered.props,
      enhance(rendered.props && rendered.props.children ? rendered.props.children : null, state, onChange, statePath)
    );
  }
}
